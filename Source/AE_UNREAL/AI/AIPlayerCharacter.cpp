...
void AAIPlayerCharacter::BeginPlay()
{
	...
	TArray<UActorComponent*> WeaponMesh = GetComponentsByTag(USceneComponent::StaticClass(), TEXT("WeaponMesh"));
	UStaticMeshComponent* MyMesh = Cast<UStaticMeshComponent>(WeaponMesh[0]);
	MyMesh->OnComponentBeginOverlap.AddDynamic(this, &AAIPlayerCharacter::BeginOverLap);
...
void AAIPlayerCharacter::BeginOverLap(
...
)
{
	...
	if (OtherActor->ActorHasTag(TEXT("Monster")))
	{
		...
		Mon->GetBlackboardComponent()->SetValueAsBool(TEXT("isdeath"), true);
